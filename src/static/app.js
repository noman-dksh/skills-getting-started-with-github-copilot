document.addEventListener("DOMContentLoaded", () => {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length
          ? `<ul class="participants-list">${details.participants
              .map(
                (participant) =>
                  `<li><button class="participant-delete-button" data-activity="${encodeURIComponent(
                    name
                  )}" data-email="${encodeURIComponent(participant)}" aria-label="Remove ${participant}">×</button><span>${participant}</span></li>`
              )
              .join("")}</ul>`
  ...
  activitiesList.addEventListener("click", (event) => {
    ...
    if (target.classList.contains("participant-delete-button")) {
